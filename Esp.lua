...
local LEGENDARY_NAMES = {}

local function autoFillLegendary()
    ...
    local function scanForLegendary(node, depth)
        if type(node) ~= "table" or depth > 6 then return end
        
        local rarity = node.rarity or node.tier or node.quality or node.Rarity or node.Type or node.RarityTier
        if rarity then
            local rLow = tostring(rarity):lower()
            if rLow:find("legend") or rLow:find("мифическ") or rLow:find("экзотическ") then
                if node.name and node.name ~= "" then
                    LEGENDARY_NAMES[tostring(node.name):lower()] = true
                end
            end
        end
        
        if node.name then
            local nameLow = tostring(node.name):lower()
            if nameLow:find("legend") or nameLow:find("мифическ") or nameLow:find("экзотическ") then
                LEGENDARY_NAMES[nameLow] = true
            end
        end
        
        for _, c in pairs(node) do 
            if type(c) == "table" then
                scanForLegendary(c, depth + 1) 
            end
        end
    end
    
    if ClothingConfig then
        scanForLegendary(ClothingConfig, 0)
    end
    if AccessoryCfg then
        scanForLegendary(AccessoryCfg, 0)
    end
    
    local count = 0
    for _ in pairs(LEGENDARY_NAMES) do count = count + 1 end
    
    if count == 0 then
        LEGENDARY_NAMES["__fallback__"] = true
    end
end
...
local function tierFor(profit, name)
    if not profit or profit <= 0 then return nil end
    
    local lowerName = tostring(name or ""):lower()
    local isLegendary = false
    
    for legName in pairs(LEGENDARY_NAMES) do
        if legName == "__fallback__" then
            isLegendary = true
            break
        elseif string.find(lowerName, legName) then
            isLegendary = true
            break
        end
    end
...
local function drawSlot(entry, color)
    ...
    if not o then
        ...
        bb.Adornee = slotRef
        bb.AlwaysOnTop = true
        bb.Size = UDim2.fromOffset(280, 90)
        ...
        local st = Instance.new("UIStroke"); st.Thickness = 2.5; st.Parent = card
        o.stroke = st

        local nameL = Instance.new("TextLabel")
        nameL.Size = UDim2.new(1, -10, 0, 30)
        nameL.Position = UDim2.fromOffset(5, 4)
        ...
        nameStroke.Parent = nameL

        local infoL = Instance.new("TextLabel")
        infoL.Size = UDim2.new(1, -10, 0, 50)
        infoL.Position = UDim2.fromOffset(5, 36)
    ...
    if o.nameL:FindFirstChild("UIStroke") then
        o.nameL.UIStroke.Color = color
    end
    
    local buyPrice = entry.price or 0
    local sellPrice = entry.price and entry.profit and (entry.price + entry.profit) or 0
    o.infoL.Text = ("💰 Куп: $%s  |  Прода: $%s"):format(fmt(buyPrice), fmt(sellPrice))
end

local function applySlot(entry)
    if not ESP.enabled then destroyObjs(entry.slotRef); return end
    local color = tierFor(entry.profit, entry.name)
...
espBtn.Activated:Connect(function()
    ...
    refreshESP()
    notify(ESP.enabled and "ESP включён — только легендарки" or "ESP выключен")
end)
